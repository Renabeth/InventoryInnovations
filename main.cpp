...
void loadProducts(Product products[],
                  int size); // load Products.txt(product cost per lb)
...
void updateProduct(Product products[],
                   int size); // gets data to send to updateFile()
void updateStorage(Storage storagelist[],
                   int size); // gets data to send to updateFile()
void updateFile(
    string file, string find,
    string newline); // update file when data changes(ex.purchase{add purchased
                     // item to storage},fund changes)
...
void login(User users[],
           int size); // check if inputted login exists and logs them in
void displayMenu(User users[], int usize, Product products[], int psize,
                 Storage storagelist[], int isize,
                 vector<PurchasedFood> pfs); // for members
void displayMenuA(User users[], int usize, Product products[], int psize,
                  Storage storagelist[], int isize,
                  vector<PurchasedFood> pfs); // for admin
void DisperseFunds(User users[], int size);   // for admin to distribute funds
void buyProducts(User users[], int usize, Product products[], int psize,
                 Storage storagelist[], int isize,
                 vector<PurchasedFood> pfs);    // buy product then update files
...
bool DeptLimit(User users[],
               int choice); // Limit what dept members can buy in buyProducts()
...
int main() {
  ...
  loadProducts(products,
               PRODUCTSIZE); // enter/load product information into array
  ...
  loadStorage(storagelist,
              STORAGESIZE); // enter/load Inventory information into array
  ...
  if (users[lif].getDepartment() == 0) // if person is admin display diff menu
    displayMenuA(users, USERSIZE, products, PRODUCTSIZE, storagelist,
                 STORAGESIZE, pfs);
  else
    displayMenu(users, USERSIZE, products, PRODUCTSIZE, storagelist,
                STORAGESIZE, pfs);
...
void login(User users[], int size) {
  ...
  do {
    ...
    for (int i = 0; i < size; i++) {
      if (enterUsername == users[i].getUsername() &&
          enterPass == users[i].getPassword()) {
...
void displayMenu(User users[], int usize, Product products[], int psize,
                 Storage storagelist[], int isize, vector<PurchasedFood> pfs) {
  int choice = 0;
  ...
  do { // loops displayMenu until they log out
    ...
    GoodInputTest(
        choice); // If letter returns false but also resets choice to zero
    switch (choice) {
    case 1: {
      viewInventory();
      break;
...
void buyProducts(User users[], int usize, Product products[], int psize,
  ...
  do {
    ...
    if (!DeptLimit(users, choice)) { // check Dept purchase authority
      cout << "Your Department does not allow for purchase of this product!"
           << endl;
      continue;
    }

    cout << "\nTotal Funds available: $" << users[lif].getFunds() << endl;
    cout << "How much do you want to buy?(lb): costs $"
         << products[i].getPrice() << " per lb" << endl;
    ...
    if (calcprice > users[lif].getFunds() || quantity < 0) {
      ...
      cout << "Error: Non number entered or insufficient funds for "
              "purchase!\nTry again."
           << endl;
    } else {
      users[lif].setFunds(users[lif].getFunds() - calcprice);
      storagelist[i].setQuantity(storagelist[i].getQuantity() + quantity);
      pfs.push_back(PurchasedFood(products[i].getProductID(), quantity,
                                  calcprice, getDate()));
...
void viewTransactions() { // view last 10 Transactions
  ...
  while (!infile.eof() && !EmptyFileTest("Transactions.txt")) {
    if (line == start ||
        line <= 0) // check if it's the last 10 lines then displays them
    {
  ...
  if (EmptyFileTest("Transactions.txt")) {
    cout << "No Previous Transactions to view" << endl;
  }
  cout << "\n*********************************************" << endl;
  ...
  do { // loops if invalid choice/input
    ...
    case 1: {
      ...
      do { // loops if invalid input
        cout << "\nFunds available to disperse: $" << users[lif].getFunds()
             << endl;
        cout << "Amount: $";
        amount = 0;
        ...
        if (amount > users[lif].getFunds() || amount < 0) {
          cout << "Error. Non number entered or insufficient funds.Try again."
               << endl;
          again = true;
        } else {
    ...
    case 2: {
      ...
      do { // loops if invalid input
        cout << "Funds available to disperse: $" << users[lif].getFunds()
             << endl;
      ...
      users[lif].setFunds(users[lif].getFunds() -
                          amount); // deducts amount from Admin
      users[dep].setFunds(users[dep].getFunds() +
                          amount); // increases funds of member
      break;
...
void loadUsers(User users[], int size) { // open a file in read mode.
  ...
  while (
      !infile
           .eof()) // goes through file adding all fields to a User class array
  {
...
void loadProducts(Product products[], int size) { // open a file in read mode
  ...
  while (!infile.eof()) // goes through file adding all fields to a Product
                        // class array
  {
    getline(infile, I, ',');
    productID = stol(I);
...
void loadStorage(Storage storagelist[], int size) { // open a file in read mode
  ...
  while (!infile.eof()) // goes through file adding all fields to a Storage
                        // class array
  {
    getline(infile, I, ',');
    productID = stol(I);
...
void updateUser(
    User users[],
    int size) { // gets current users[] data and sends to updateFile() to update
  ...
  for (int i = 0; i < size; i++) {
    ...
    find += users[i].getFullname() +
            ","; // search line for "find" string to change(this changes data
                 // based on fullname)
    ...
    if (users[lif].getDepartment() == 0) { // If admin then show updated info
      ...
      cout << "$" << users[i].getFunds()
           << endl; //(Not needed) Displays updated Funds
    }
...
void updateProduct(Product products[],
                   int size) { // gets current products[] data and sends to
                               // updateFile() to update
  ...
  for (int i = 0; i < size; i++) {
    ...
    find += to_string(products[i].getProductID()) +
            ","; // search line for "find" string to change(this changes data
                 // based on ProductID)
...
void updateStorage(Storage storagelist[],
                   int size) { // gets current storagelist[] data and sends to
                               // updateFile() to update
  ...
  for (int i = 0; i < size; i++) {
    ...
    find += to_string(storagelist[i].getProductID()) +
            ","; // search line for "find" string to change(this changes data
                 // based on ProductID)
...
void updateFile(string file, string find,
                string newline) { // update file contents
  ...
  while (getline(myfile2, currentline)) {
    if (line == dataEnd) {
      n = "";
    } // if this is last line set n="" to prevent additional line being created

    if (currentline.find(find) == string::npos &&
        !currentline.empty()) // check if line needs to be changed(if it
                              // contains "find" string)
    {
      buffer += currentline + n;
    } // if no change needed leave line(enter old line)

    else if (!currentline.empty()) {
      buffer += newline + n;
    } // change old line to newline
...
void DeleteEmptyLines(string filename) { // deletes empty lines
  ...
  ofstream temp;
  temp.open("temp.txt"); // create file temp.txt to place new/updated data with
                         // deleted lines
...
void writePurchases(vector<PurchasedFood> pfs) { // open a file in write mode.
  ...
  DeleteEmptyLines("Transactions.txt");
} // end of function? (unsure how exactly works,was told the way it worked that
...
bool DeptLimit(
    User users[],
    int choice) { // sets what choices each dep can choose (for BuyProducts)
